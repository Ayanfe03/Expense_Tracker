...
const getExpensesHandler = async (req, res) => {
  try {
    const user = req.user;
    let { search, filter, startDate, endDate } = req.query; // Query params used for optional features that you might not use everytime
    ...
    if (startDate && endDate) {
      ...
      // add an extra day to the endDate
...
const getExpenseSummaryHandler = async (req, res) => {
  try {
    let { startDate, endDate } = req.query; // Query params used for optional features that you might not use everytime
    const user = req.user;
    let expenses = [];
...
const downloadExpenseStatementHandler = async (req, res) => {
  try {
    ...
    res.download(filePath, 'expenses.csv', (error) => {
      ...
      fs.unlinkSync(filePath); // deletes the csv file
    });
  } catch (error) {
    return res.status(500).json({
      message: error.message,
